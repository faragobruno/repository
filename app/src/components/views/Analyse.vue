<template>
  <div class="analyse-view">
    <ul class="list-view">
      ...
      <h1></h1>
      <li>Number of bookings/goal: {{calMax}}/{{month}}</li>
      <li>Number of bookings in the first quarter: {{firstQuar}}</li>
      <li>Number of bookings in the second quarter: {{secondQuar}}</li>
      <li>Number of bookings in the third quarter: {{thirdQuar}}</li>
      <li>Number of bookings in the fourth quarter: {{fourthQuart}}</li>
      <li>Number of bookings in the first half: {{firstHalf}}</li>
      <li>Number of bookings in the second half: {{secondHalf}}</li>
    </ul>
    ...
    <b-progress :max="calMax" class="mt-2" variant="success">
      <b-progress-bar :value="secondQuar" :label="`${((secondQuar / calMax) * 100).toFixed(2)}%`"></b-progress-bar>
    </b-progress>
    <b-progress :max="calMax" class="mt-2" variant="success">
      <b-progress-bar :value="thirdQuar" :label="`${((thirdQuar / calMax) * 100).toFixed(2)}%`"></b-progress-bar>
    </b-progress>
    <b-progress :max="calMax" class="mt-2" variant="warning">
      <b-progress-bar :value="fourthQuart" :label="`${((fourthQuart / calMax) * 100).toFixed(2)}%`"></b-progress-bar>
    </b-progress>
    <b-progress :max="calMax" class="mt-2" variant="success">
      <b-progress-bar :value="firstHalf" :label="`${((firstHalf / calMax) * 100).toFixed(2)}%`"></b-progress-bar>
    </b-progress>
    <b-progress :max="calMax" class="mt-2" variant="warning">
      <b-progress-bar :value="secondHalf" :label="`${((secondHalf / calMax) * 100).toFixed(2)}%`"></b-progress-bar>
    </b-progress>
  </div>
</template>
  ...
  methods: {
    async getEvents() {
      ...
      calsnapshot.forEach(doc => {
        if (doc.data().start > "2019-01-01" && doc.data().end < "2019-03-31") {
          this.firstQuar++;
        } else if (
          doc.data().start > "2019-04-01" &&
          doc.data().end < "2019-06-30"
        ) {
          this.secondQuar++;
        } else if (
          doc.data().start > "2019-07-01" &&
          doc.data().end < "2019-09-30"
        ) {
          this.thirdQuar++;
        } else if (
          doc.data().start > "2019-10-01" &&
          doc.data().end < "2019-12-31"
        ) {
          this.fourthQuart++;
        }
        if(doc.data().start > "2019-01-01" && doc.data().end < "2019-05-31"){
          this.firstHalf++;
        }
        else{
          this.secondHalf++;
